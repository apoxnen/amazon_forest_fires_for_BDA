...
data {
  ...
  int pred_index;    // input location for prediction
}
...
generated quantities {
  real ypred;
  vector[N] log_lik;                                            // added line for this assignment
  ypred = normal_rng(mu[pred_index], sigma[pred_index]);
  for (i in 1:N)                                           
    log_lik[i] = normal_lpdf(y[i] | mu[x[i]], sigma[x[i]]);     // added line for this assignment
}
...
### Point 4
Assess how reliable the PSIS-LOO estimates are for the three models based on the k-values.

Answer:
The above results for k-value (first three lines) and PSIS-LOO (in the last line) are collected in the following table:
(of course the results change avery time the code is run, so they could be a little bit different from the above reported)


PSIS-LOO is useful since allow not to refitting K times the model, like K-fold cross validation requires.
If all the $$\hat{k} < 0.7 $$, the PSIS-LOO estimate can be considered to be reliable,
otherwise there is a concern that it may be biased (too optimistic, overestimating the predictive accuracy of the model).
So, in case of $$\hat{k} > 0.7 $$, if there are just a few i, it is recommended sampling directly from each corresponding $$p(\theta^s|y_{-i}) $$.
If there are a lot, the results should be checked using K-fold cross-validation.


About the previous results, the $$\hat{k}$$ for the pooled and hierarchical models are all below 0.7. This means that the PSIS-LOO estimate is reliable.
For the separate model, the obtained $$\hat{k} < 0.7 $$ are not all below 0.7. This means that the PSIS-LOO may be biased. But the values that don't respect this rule are a few ( 6%).




### Point 5
An assessment of whether there are differences between the models with regard to
the elpdloo-cv, and if so, which model should be selected according to PSIS-LOO.

Answer:
According to PSIS-LOO, the model with the highest value is selected. As reported above in the table and in view of the previous discussion, the hierarchical model should be selected.
