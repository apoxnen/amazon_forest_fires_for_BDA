...
# regression model w.r.t the states of Brazil and sum of number of fires per year
...
N <- 460          # N of observations per state
K <- 23           # N of states
xpred <- 24
data=list(N =N, K=K, x=x, y=y, xpred = xpred)
...
stan_dens(fit2, pars=c("y_prep"))

pred <- as.data.frame(extract(fit2, "y_prep"))
colMeans(pred)
