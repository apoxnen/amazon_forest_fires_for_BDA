...
For the negative binomial model, as well as the regression model, we used the priors $\alpha = Exp(0.0006303)$ and $\beta = Exp(1.2)$. For $\alpha$ we used $1/var(dataset)$ as $\lambda$ for the exponential distribution. The $\lambda$ of $\beta$ was acquired by experimenting with different priors.
...
#6 Results
Here the PSIS-LOO elpd values and the k-values for each of the three normal models and the two negative binomial models introduced in the last section as well as the effective number of parameters Peff for each of the model.
